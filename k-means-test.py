...
# describe_clusters (K-Meansクラスタ命名用) で使用するキーワードのフラットリスト
# 上記の辞書のすべての値を結合して作成
...
class ImageSorter:
    ...
    def _load_clip_model(self):
        """CLIPモデルとプリプロセッサ、トークナイザーをロードします。"""
    ...
    def _prepare_keyword_features(self):
        ...
        self.keyword_classification_map = {} # {カテゴリ名: そのカテゴリの代表キーワード特徴量}
    ...
    def find_best_k(self, features_np, k_range=range(10, 21)):
        ...
        if len(features_np) < 2:
            return 1 # クラスタリングできない場合は1を返す（全画像を1つのクラスタとして扱う）
        
        # k_rangeをデータ点数で調整
        if len(features_np) < k_range.stop:
            k_range = range(k_range.start, max(2, len(features_np) + 1)) # データ点数まで（+1でrangeの終端を調整）
        
        # 最小kがデータ点数を超える場合
        if k_range.start >= len(features_np):
            return max(1, len(features_np) // 2) # 適当なクラスタ数を返すか、1を返す
        ...
        for k in k_range:
            try:
                ...
                if len(set(labels)) < 2 or len(set(labels)) > len(features_np):
                    # クラスタが1つしかない、またはクラスタ数がデータ点数より多い場合はスコア計算不可
                    continue
                ...
                continue
        return best_k if best_k > 0 else max(2, len(features_np) // 5) # 少なくとも2以上のクラスタ、またはデータ数の1/5
    ...
    def run_hybrid_classification(self, image_paths, args_k):
        ...
        final_labels = np.full(len(image_paths), None, dtype=object) # Noneで初期化
        final_cluster_names = {}
        ...
        if len(unclassified_features_np) > 0:
            ...
            if best_k_for_kmeans > 0:
                ...
                for i, kmeans_label in enumerate(kmeans_labels_unclassified):
                    ...
                    # キーワード分類のラベルと重複しないように、ユニークなキーを生成
        ...
        return final_labels, final_cluster_names
    ...
    def organize_images(self, image_paths, labels, cluster_names, output_dir):
        """
        分類結果に基づいて画像をフォルダに移動します。
        """
...
def main():
    parser = argparse.ArgumentParser(description="画像分類ツール:画像ファイルをフォルダに振り分ける")
    parser.add_argument('--max', type=int, default=100000,
                        help='読み込む画像の最大枚数（デフォルト: 100000）')
    parser.add_argument('--dir', type=str, nargs='+', default=['.'], # nargs='+' で複数指定可能に
    ...
    parser.add_argument('--kmeans-only', action='store_true', dest='kmeans_only',
                        help='K-Meansのみで分類する場合は指定（CLIP特徴量を使用）') # K-Means専用モード
    ...
    if args.size:
        labels, cluster_names = sorter.run_size_classification(image_paths)
        output_dir = "clusters_by_size"
    elif args.kmeans_only:
        labels, cluster_names = sorter.run_kmeans_only_classification(image_paths, args.k)
        output_dir = "clusters_by_kmeans_only"
    else: # デフォルト動作: ハイブリッドモード
        labels, cluster_names = sorter.run_hybrid_classification(image_paths, args.k)
        output_dir = "clusters_by_hybrid_clip"

    # 画像の移動
    sorter.organize_images(image_paths, labels, cluster_names, output_dir)
